...
export function mapImageToDataURL(map, icon) {
    ...
    ctx.putImageData(new ImageData(Uint8ClampedArray.from(image.data.data), image.data.width, image.data.height), 0, 0);
...
export function loadImage(url, { transformRequest }) {
    const fetchObj = Object.assign({}, transformRequest(url));
    if (fetchObj.headers) {
